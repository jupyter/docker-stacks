![docker pulls](https://img.shields.io/docker/pulls/jupyter/ruby-notebook.svg) ![docker stars](https://img.shields.io/docker/stars/jupyter/ruby-notebook.svg) [![](https://images.microbadger.com/badges/image/jupyter/ruby-notebook.svg)](https://microbadger.com/images/jupyter/ruby-notebook "jupyter/ruby-notebook image metadata")

# Jupyter Notebook Ruby Stack
...
```
docker run -it --rm -p 8888:8888 jupyter/ruby-notebook
```
...
```
docker run -d -p 8888:8888 jupyter/ruby-notebook start-notebook.sh --NotebookApp.password='sha1:74ba40f8a388:c913541b7ee99d15d5ed31d4226bf7838f83a50e'
```

For example, to set the base URL of the notebook server, run the following:

```
docker run -d -p 8888:8888 jupyter/ruby-notebook start-notebook.sh --NotebookApp.base_url=/some/path
```

For example, to disable all authentication mechanisms (not a recommended practice):

```
docker run -d -p 8888:8888 jupyter/ruby-notebook start-notebook.sh --NotebookApp.token=''
```
...
docker run -d -p 8888:8888 \
    ...
    jupyter/ruby-notebook start-notebook.sh \
...
The `start.sh` script supports the same features as the default `start-notebook.sh` script (e.g., `GRANT_SUDO`), but allows you to specify an arbitrary command to execute. For example, to run JupyterLab instead of the classic notebook, run the following:

```
docker run -it --rm -p 8888:8888 jupyter/ruby-notebook start.sh jupyter lab
```
